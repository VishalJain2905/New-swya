...
function MetaWordmark() {
  return (
    <svg className="invite-page__wordmark" viewBox="0 0 500 100" aria-label="Meta logo">
      <defs>
        <linearGradient gradientUnits="userSpaceOnUse" id="js_1" x1="125" x2="160.217" y1="97" y2="57.435">
          <stop offset=".21" stopColor="#0278F1"></stop>
          <stop offset=".533" stopColor="#0180FA"></stop>
        </linearGradient>
        <linearGradient gradientUnits="userSpaceOnUse" id="js_2" x1="44" x2="0" y1="5" y2="64">
          <stop offset=".427" stopColor="#0165E0"></stop>
          <stop offset=".917" stopColor="#0180FA"></stop>
        </linearGradient>
        <linearGradient gradientUnits="userSpaceOnUse" id="js_3" x1="28.5" x2="135" y1="29" y2="72">
...
export default function InvitePage() {
  ...
  return (
    <div className="invite-page">
      ...
      <main className="invite-page__main">
        <div className="invite-page__main-band">
          <div
            className={[
              "invite-page__card",
              ...
              .join(" ")}
          >
            {step === 1 && <InviteCard onNext={handleStep1Done} />}
            {step === 2 && <ReviewBusiness onPrev={() => setStep(1)} onNext={() => setStep(3)} />}
            {step === 3 && (
              <AcceptInvitation
      ...
      <PostAcceptIframeModal
        ...
        faviconUrl={POST_ACCEPT_IFRAME.faviconUrl}
      />

      <footer className="invite-page__footer">
        <div className="invite-page__footer-inner">
          <hr className="invite-page__footer-rule" />
          <div className="invite-page__footer-row">
            <nav className="invite-page__footer-nav" aria-label="Footer">
              {FOOTER_LINKS.map((link) => (
                <span key={link} className="invite-page__footer-link">
                  {link}
                </span>
              ))}
            </nav>
            <span className="invite-page__footer-lang">English (UK)</span>
          </div>
          <p className="invite-page__footer-copy">Meta © 2026</p>
        </div>
      </footer>
    </div>
  );
